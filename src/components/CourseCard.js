...
const CourseCard = ({course}) => {
    ...
    const {name, id, price, duration, img} = course;

    return (
        <Link to={`/courseDetails/${id}`} className="flex flex-col bg-white rounded-md dark:bg-gray-900 dark:border-white dark:border-2 shadow-md overflow-hidden"  >
            ...
            <div className="flex flex-col flex-1 p-6"  >
                ...
                <div className="flex flex-wrap justify-between pt-3 space-x-2 text-xs text-black dark:dark:text-gray-400"  >
                    <span className="text-black dark:text-violet-400" >{duration} hr</span>
                    <span className="text-black dark:text-violet-400" >{price} $</span>
                </div>
            </div>
        </Link>
    );
